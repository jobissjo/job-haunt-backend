...
from apis.utils.common_utils import CustomTranslatedException
import logging
...
def custom_exception_handler(exc, context):
    ...
    if request:
        ...
        if isinstance(exc, CustomTranslatedException):
            log_message = f"{request.method} {request.path} - Custom Exception: {exc.error_message}"

        logger.error(log_message)

    if isinstance(exc, CustomTranslatedException) and response is not None:
        response.data = {
            "error": exc.error_message,
        }
        response.status_code = exc.status_code
    elif isinstance(exc, CustomTranslatedException):
